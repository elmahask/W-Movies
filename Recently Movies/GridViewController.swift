...
private var API_URL = "https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key="+API_KEY;
...
class GridViewController: UICollectionViewController {
    ...
    @IBAction func btnChangeMovie(_ sender: UIBarButtonItem) {
        ...
        if(flag){
            //sortArray(sortMovie: movieList)
            //movieList.sorted(by:{$0.popularity! < $1.popularity!});
            ...
            Alamofire.request(API_URL).responseJSON { (response) in
                if let responseValue = response.result.value as! [String: Any]? {
                    ...
                    for item in self.responseJson{
                        ...
                        movie.image = poster_URL + imageURL
